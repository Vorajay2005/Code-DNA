// Function to force GitHub to show the login page
function forceGitHubLogin() {
    console.log("Switch Account button clicked");
    
    // Show a loading indicator
    const switchAccountBtn = document.querySelector('a[href="index.php?new_login=1"]');
    if (switchAccountBtn) {
        switchAccountBtn.innerHTML = '<i class="fas fa-spinner fa-spin"></i> Switching...';
        switchAccountBtn.style.pointerEvents = 'none';
    }
    
    // First, clear our session and get a special GitHub auth URL
    fetch('force_github_login.php', {
        method: 'GET',
        headers: {
    ...
    .then(data => {
        ...
        if (data.success) {
            ...
            setTimeout(() => {
                // Redirect to GitHub's login page with special parameters
                window.location.href = data.url;
            }, 500);
        } else {
            console.error("Error from server:", data.error);
            alert('Failed to switch accounts: ' + (data.error || 'Unknown error'));
            // Reset the button
            if (switchAccountBtn) {
                switchAccountBtn.innerHTML = '<i class="fas fa-exchange-alt"></i> Switch Account';
                switchAccountBtn.style.pointerEvents = 'auto';
            }
        }
    })
    .catch(error => {
        console.error('Fetch error:', error);
        alert('An error occurred while trying to switch accounts. Please try again or log out manually.');
        // Reset the button
        if (switchAccountBtn) {
            switchAccountBtn.innerHTML = '<i class="fas fa-exchange-alt"></i> Switch Account';
            switchAccountBtn.style.pointerEvents = 'auto';
...
document.addEventListener('DOMContentLoaded', () => {
    // Add event listener for the switch account button
    const switchAccountBtn = document.querySelector('a[href="index.php?new_login=1"]');
    if (switchAccountBtn) {
    ...
    if (document.getElementById('activityChart')) {
        ...
        for (let day = 0; day < 7; day++) {
            for (let hour = 0; hour < 24; hour++) {
                ...
                heatmapData[day][hour] = (dayOfWeekData[day] / 10) * (hourOfDayData[hour] / 5);
            }
        }
                    ...
                    tooltip: {
                        callbacks: {
                            label: function(context) {
                                ...
                                return `${day} at ${hourLabel}: ~${Math.round(point.v)} commits`;
                            }
                        }
                    }
                },
                    ...
                    x: {
                        ...
                        ticks: {
                            callback: function(value) {
                                if (value % 6 === 0) {
                                    return value < 12 ? 
                                        (value === 0 ? '12am' : `${value}am`) : 
                                        (value === 12 ? '12pm' : `${value-12}pm`);
                                }
                                return '';
                            },
                            color: '#c9d1d9'
    ...
    if (shareBtn && shareContainer) {
        shareBtn.addEventListener('click', () => {
            ...
            watermark.innerHTML = '<p>Generated with GitHub Wrapped | github-wrapped.com</p>';
        ...
        twitterBtn.addEventListener('click', () => {
            html2canvas(sharePreview.firstChild).then(canvas => {
                ...
                const tweetText = `Check out my GitHub Wrapped! I made ${stats?.total_commits || 'many'} commits this year and my top language is ${languageLabels[0] || 'awesome'}. #GitHubWrapped`;
